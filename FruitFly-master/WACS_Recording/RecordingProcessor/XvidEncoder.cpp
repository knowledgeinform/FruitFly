...
static unsigned char *mp4_buffer = NULL;
static unsigned char *in_buffer = NULL;
static unsigned char *out_buffer = NULL;

static double enctime;
static double totalenctime = 0.;
static float totalPSNR[3] = {0., 0., 0.};
...
static int m4v_size;
static int key;
static int stats_type;
static int stats_quant;
static int stats_length;
static int use_assembler = 1;
static int fakenvop = 0;
static int i;
static int nvop_counter;
...
static int avierr;
static PAVIFILE myAVIFile=NULL;
static PAVISTREAM myAVIStream=NULL;
static AVISTREAMINFO myAVIStreamInfo;
static BITMAPINFOHEADER myBitmapInfoHeader;
#endif
...
void InitEncoding(const char* const szOutputFileName, const int nFrameWidth, const int nFrameHeight)
{
	...
	if(ME_ELEMENTS != VOP_ELEMENTS) {
		...
		return;
			...
			ARG_AVIOUTPUTFILE = (char*)szOutputFileName;
			XDIM = nFrameWidth;
			YDIM = nFrameHeight;
#else
 ...
 ****************************************************************************/
	...
	if (get_frame == NULL) 
	{
		...
		//
		// Since we are encoding in grayscale, the U and V values will be the same 
		// for all frames.  Therefore we can pre-load them here.
		//
		int R, G, B, Y, U, V;
		R = G = B = 255;
		...
		unsigned char *pU = in_buffer + XDIM * YDIM;
		unsigned char *pV = in_buffer + XDIM * YDIM + XDIM / 2 * YDIM / 2;
		for (i = 0; i < YDIM / 2; i++) 
		{
			memset(pU, U, XDIM / 2);
			memset(pV, V, XDIM / 2);
...
void EncodeFrame(const unsigned char* const pImage)
{
	memcpy(in_buffer, pImage, XDIM * YDIM);
