...
public:
	...
	PointSet(const string &path, int numextrafields = 0);

	//!Constructor for delimited ASCII
	PointSet(const string &path, int numheaderrows, char delim, int numextrafields = 0, double* offsets = NULL);

	//!Constructor for Empty PointSet
	PointSet(const string &path,int npoints, int numextrafields);
	...
	Point3D convertToProjection(double origOffsetX, double origOffsetY,double zRatio = 20, double* zOffset=NULL, bool nearestPointOnly=false );
	
	//!Determine the transform to put point cloud into a virtual camera's reference frame
	DMatrix createWorldToViewTransform(double lookAtX, double lookAtY, double lookAtZ, double eyeX, double eyeY, double eyeZ);
	...
	void applyTransformToPoints(bool reapplyOffsets = true);
	...
	int xyz_to_ij(double x, double y, double z);

	//!reverse calibration method to get row, column from xyz - alternate return method
	int xyz_to_ij(double x, double y, double z, int& ii, int& jj);
