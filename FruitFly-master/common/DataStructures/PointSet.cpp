//---------------------------------------------------------------------------------------
/*!\class  File Name:	PointSet.cpp
// \author Author:		Myron Brown, Jason A.Stipes
// \brief  Purpose:		Class for 3D Point Sets
//
// \note   Notes:	
//
// \note   Routines:  see PointSet class
//
...
/*! \fn PointSet::PointSet(const string &path, int numextrafields)
 *  \brief Constructor for reading in a Binary file
 *  \param path - The path to read/store the pointset
 *  \param numextrafields - The number of extra fields (in addition to x,y,z)
 *  \exception 
 *  \return void
 */
PointSet::PointSet(const string &path, int numextrafields)
:DataFile(path,sizeof(pSetInfo),(char*) &pSetInfo,numextrafields+3), transform()
{
	init(true);
}

/*! \fn PointSet::PointSet(const string &path, int numheaderrows, char delim, int numextrafields, double* offsets)
 *  \brief Constructor for delimited ASCII
 *  \param path - The path to read/store the pointset
 *  \param numheaderrows - The number of ASCII header rows to skip
 *  \param delim -  The delimiting character
 *  \param numextrafields - The number of extra fields (in addition to x,y,z)
 *  \param offsets - The offsets to subtract from any data read in from the ASCII file.  
					 By default, the first X and Y value (floor) will be used as an offset)
 *  \exception 
 *  \return void
 */
PointSet::PointSet(const string &path, int numheaderrows, char delim, int numextrafields, double* offsets)
:DataFile(path, numheaderrows, delim, numextrafields+3, offsets), transform()
{
	init(true);
}

/*! \fn PointSet::PointSet(const string &path,int npoints, int nfields)
 *  \brief Constructor for creating a new PointSet
 *  \param path - The path to read/store the pointset
 ...
 *  \return void
 */
PointSet::PointSet(const string &path,int npoints, int numextrafields)
:DataFile(path,npoints,numextrafields+3)
{
	init(false);
}

/*! \fn void PointSet::init(bool hasData)
 *  \brief Constructor for creating a new PointSet
 *  \param hasData - Indicates whether this init is for a new Pointset, or one read from file(true)
 *  \exception 
 *  \return void
 */
void PointSet::init(bool hasData)
{
...
/*! \fn bool PointSet::Read(char *fileName, bool readData)
 ...
 */
bool PointSet::Read(char *fileName, bool readData)
{
...
/*! \fn bool PointSet::Write(const char *fileName)
 *  \brief Writes a binary file and initializes data structures 
 *  \param fileName - The file to write
 *  \exception 
 *  \return boolean success
 */
bool PointSet::Write(const char *fileName)
{
	this->writeBinaryFile(fileName);
	return true;
}

/*! \fn void PointSet::getTransformedPoints(float*x, float*y, float*z, float refx, float refy, float refz)
 ...
 *  \return void
 */
void PointSet::getTransformedPoints(float*x, float*y, float*z, float refx, float refy, float refz)
{
...
/*! \fn void PointSet::applyTransformToPoints()
 *  \brief Applies the PointsetTransform to all points in the PointSet
 *         The bounding box is not updated.  Data will be re-offset
 *		   if the flag is set.
 *  \param reapplyOffsets - boolean flag, true if offsets should be reapplied
 *  \exception 
 *  \return void
 */
void PointSet::applyTransformToPoints(bool reapplyOffsets)
{
	double xx, yy, zz;
	for(int i=0; i<this->numPoints; i++)
	{
		...
		if (reapplyOffsets)
		{
			x[i] = (float) (xx - (double)xoff);
			y[i] = (float) (yy - (double)yoff);
			z[i] = (float) (zz - (double)zoff);
		}
		else
		{
			x[i] = (float) (xx);
...
/*! \fn void PointSet::ApplyTransform(PointSetTransform transform, bool applyOffset)
 *  \brief Applies the PointsetTransform to all points in the PointSet,updates bounding box
 ...
 *  \return void
 */
void PointSet::ApplyTransform(PointSetTransform transform, bool applyOffset)
{
	double xs;
	double ys;
	double zs;
	...
	R = pst.angles2rot(transform.rx, transform.ry, transform.rz);
...
/*! \fn int PointSet::xyz_to_ij(float x, float y, float z)
 ...
 */
int PointSet::xyz_to_ij(double x, double y, double z)
{
		int xx, yy;
		if(projection)
		{
 ...
 *  \return void
 */
int PointSet::xyz_to_ij(double x, double y, double z, int& ii, int& jj)
{
...
/*! \fn void PointSet::ijz_to_xy(float i, float j, float z, float&xx , float&yy)
 ...
 *  \return void
 */
void PointSet::ijz_to_xy(float i, float j, float z, float&xx , float&yy)
{
		if(projection)
		{
...
/*! \fn DMatrix PointSet::createWorldToViewTransform(float lookAtX, float lookAtY, float lookAtZ, float eyeX, float eyeY, float eyeZ)
 ...
 */
DMatrix PointSet::createWorldToViewTransform(double lookAtX, double lookAtY, double lookAtZ, double eyeX, double eyeY, double eyeZ)
{
...
/*! \fn float* PointSet::clipToBounds(PointSetBounds* bounds, int& r, int&c, int& ci, int& cj)
 ...
 */
float* PointSet::clipToBounds(PointSetBounds* bounds, int& r, int&c, int& ci, int& cj)
{
	float* retval;
...
/*! \fn Point3D PointSet::convertToProjection(float origOffsetX, float origOffsetY, float zRatio, float* zOffset, bool nearestPointOnly )
 ...
 */
Point3D PointSet::convertToProjection(double origOffsetX, double origOffsetY, double zRatio, double* zOffset, bool nearestPointOnly )
{
	...
	{
		offsetZ = max(bounds.xmax-bounds.xmin, bounds.ymax-bounds.ymin) * zRatio;
		if(zOffset != NULL)
			*zOffset = offsetZ;
	}	
	...
	transform.csx = (bounds.xmax + bounds.xmin) / 2.0;
	transform.csy = (bounds.ymax + bounds.ymin) / 2.0;
	transform.csz = (bounds.zmax + bounds.zmin) / 2.0;
	...
	double xtemp, ytemp, ztemp,xtemp1, ytemp1, ztemp1;
	xtemp = bounds.xmax;
	ytemp = transform.csy;
	ztemp = transform.csz;
	xtemp1 = bounds.xmin;
	...
	//MultiCoreAlgorithm<void> m(&PointSet::projectionLoop, 0, numPoints);
	double xx,yy,zz;
...
//void PointSet::projectionLoop(int start, int stop, int step, void)
//{
//	
//	int i, ix;
//	int hit = 0;
...
/*! \fn void PointSet::calculateNormals()
	...
	for(i=0; i<rows; i++)
	{
		for(j=0; j<cols; j++)
		{
			...
			if (points[i*cols + j].count > 0)
			{
				...
				ix = y1*cols+(x1);
				if(ix <0 || ix > numpts) continue; 
				if(points[ix].count < 1)
				{
					basex = x1;
					basey = y1;
...
/*! \fn bool PointSet::CopySubset(PointSet &pset, bool *keep)
 ...
 *  \return boolean success
 */
bool PointSet::CopySubset(PointSet &pset, bool *keep)
{
	// Count the points.
	numPoints = 0;
...
/*! \fn void PointSet::createSpecialVRML(char* path, PointSet* ps1)
 ...
 *  \return void
 */
void PointSet::createSpecialVRML(char* path, PointSet* ps1) 
{
	FILE *fxx;
	int i, numPrinted=0;
	float r, g, b;
...
/*! \fn void PointSet::createVRML(char* path, PointSet* ps1, bool shownormals, bool applyxf) 
 *  \brief This function converts the PointSet into a VRML file
 *  \param path - the filename in which to store the VRML file
 *  \param ps1 - The pointset to convert to VRML
 ...
 *  \return void
 */
void PointSet::createVRML(char* path, PointSet* ps1, bool shownormals, bool applyxf) 
{
	FILE *fxx;
	int i, numPrinted=0;
	float r, g, b;
...
/*! \fn void PointSet::createVRML(char* path, PointSet** psets, int numPointSets, int* hues, bool shownormals, bool applyxf) 
 ...
 *  \return void
 */
void PointSet::createVRML(char* path, PointSet** psets, int numPointSets, int* hues, bool shownormals, bool applyxf) 
{
	FILE *fxx;
...
/*! \fn void PointSet::createVRML(char* path, PointSet* ps1, PointSet* ps2, bool shownormals, bool applyxf) 
 ...
 *  \return void
 */
void PointSet::createVRML(char* path, PointSet* ps1, PointSet* ps2, bool shownormals, bool applyxf) 
{
	FILE *fxx;
	int i, numPrinted=0;
	float r, g, b;
...
/*! \fn void PointSet::createCorrelatedVRML(char* path, PointSet* ps1, PointSet* ps2, bool applyxf )
 *  \brief This function plots two PointSets into a single VRML file and draws lines between point correspondences
 *  \param path - the filename in which to store the VRML file
 *  \param ps1 - The first pointset
 *  \param ps1 - The second pointset
 ...
 *  \return void
 */
void PointSet::createCorrelatedVRML(char* path, PointSet* ps1, PointSet* ps2, bool applyxf ) 
{
	FILE *fxp;
	int i, numPrinted=0;
	float r, g, b;
...
/*! \fn void PointSet::ApplyNoise(float sigma)
 ...
 *  \return void
 */
void PointSet::ApplyNoise(float sigma)
{
	if (sigma == 0.0) return;
	long seed = 0;
	for (int m=0;m<numPoints;m++)
...
/*! \fn bool PointSet::ConvertASCIIFolder(char *folderName, int utmZone, int numExtraVals, bool *useOffset, int numHeaderLines, bool computeSpacing)
 *  \brief This function plots two PointSets into a single VRML file and draws lines between point correspondences
 ...
 *  \return boolean success
 */
bool PointSet::ConvertASCIIFolder(char *folderName, int utmZone, int numExtraVals, bool *useOffset, int numHeaderLines, bool computeSpacing)
{
	//// Open the directory.
...
/*! \fn bool PointSet::ConvertASCIIFile(char *fileName, int utmZone, int numExtraVals, bool *useOffset, int numHeaderLines, bool computeSpacing)
 *  \brief This function plots two PointSets into a single VRML file and draws lines between point correspondences
 ...
 *  \return void
 */
bool PointSet::ConvertASCIIFile(char *fileName, int utmZone, int numExtraVals, bool *useOffset, int numHeaderLines, bool computeSpacing)
{

	PointSet* ps = new PointSet(fileName,numHeaderLines,' ',numExtraVals);
...
/*! \fn float PointSet::GetHorizontalSpacing()
 ...
 */
float PointSet::GetHorizontalSpacing()
{
